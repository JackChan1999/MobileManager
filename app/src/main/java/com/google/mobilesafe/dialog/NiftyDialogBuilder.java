...
import com.google.mobilesafe.dialog.effects.SlideButtom;
import com.google.mobilesafe.dialog.effects.SlideTop;
...
//				SlideButtom slideButtom = new SlideButtom();
//				slideButtom.setDuration(Math.abs(mDuration));
//				slideButtom.start(mLinearLayoutView);
//			}
//		});
	...
	public NiftyDialogBuilder withEffect(SlideTop slideTop) {
	...
	private void start(SlideTop slideTop) {
		//BaseEffects animator = slideTop.getAnimator();
		if (mDuration != -1) {
			slideTop.setDuration(Math.abs(mDuration));
		}
		slideTop.start(mRelativeLayoutView);
	}
